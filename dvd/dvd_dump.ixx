...
export bool redumper_dump_dvd(Context &ctx, const Options &options, DumpMode dump_mode)
{
        ...
        if(dump_mode == DumpMode::DUMP)
        {
            ...
            if(is_xbox)
            {
                ...
                if(is_xbox && !physical_structures.empty())
                {
                    ...
                    xbox_skip_ranges.push_back({ sectors_count, sectors_count + l1_padding });
    ...
    bool locked = false;
    uint32_t l1_video_start = xbox_skip_ranges[xbox_skip_ranges.size() - 1][1];
    for(uint32_t s = 0; s < sectors_count;)
    {
        ...
        // check if drive needs locking
        if(s < xbox_lock_sector && s + sectors_to_read >= xbox_lock_sector)
            sectors_to_read = std::min(sectors_to_read, xbox_lock_sector - s - 1);
        ...
        // skip xbox security sector ranges (the L1 filler is added as the last sector range)
        if(is_xbox && skip_range_idx < xbox_skip_ranges.size())
        {
        ...
        if(read)
        {
            ...
            SPTD::Status status;
            if(locked)
                status = cmd_read(*ctx.sptd, drive_data.data(), FORM1_DATA_SIZE, s - xbox_lock_sector + l1_video_start, sectors_to_read, dump_mode == DumpMode::REFINE && refine_counter);
            else
